document.addEventListener('DOMContentLoaded', function () {
    ...
    marketBlocks.forEach((block) => {
        // Находим кнопку и элементы 'market-btn-more' и 'items-share' внутри текущего блока
        const moreButton = block.querySelector('.market-btn-more');
        const itemsShare = block.querySelector('.items-share');

        // Добавляем обработчик клика для кнопки внутри текущего блока
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function closeAllSelect(elmnt) {
        var selectItems = elmnt.querySelector(".select-items").children;
        var selectedValue = elmnt.querySelector(".select-selected").getAttribute("data-value");
        for (var i = 0; i < selectItems.length; i++) {
            if (selectItems[i].getAttribute("data-value") === selectedValue) {
    ...
    for (var i = 0; i < selectItems.length; i++) {
        selectItems[i].addEventListener("click", function (e) {
            ...
            parent.querySelector(".select-selected").innerHTML = selectedText;
            var value = this.getAttribute("data-value");

            // Здесь вы можете выполнить дополнительные действия на основе выбранного значения

            // Скрываем выбранный элемент из списка
            this.style.display = "none";

            // Показываем предыдущий выбранный элемент
            var previousSelectedItem = parent.querySelector(".select-item._selected");
            if (previousSelectedItem) {
                previousSelectedItem.style.display = "flex";
                previousSelectedItem.classList.remove("_selected");
            }

            // Помечаем текущий элемент как выбранный
            this.classList.add("_selected");

            // Показываем элемент с классом 'ukraine'
            var ukraineElement = parent.querySelector(".select-item.ukraine");
            if (ukraineElement) {
                ukraineElement.style.display = "flex";
            }
        });
    }

    // Удаление элемента, который уже выбран при загрузке страницы
    var initialSelectedItem = document.querySelector(".select-item._selected");
    if (initialSelectedItem) {
        initialSelectedItem.style.display = "none";
    }

    // Скрытие элемента с классом 'ukraine' при загрузке страницы
    var ukraineElement = document.querySelector(".select-item.ukraine");
    if (ukraineElement) {
        ukraineElement.style.display = "none";
    }

    // Закрываем все селекты при клике за пределы
    ...
    function closeAllSelects() {
        var customSelects = document.getElementsByClassName("custom-select");
        for (var i = 0; i < customSelects.length; i++) {
            customSelects[i].querySelector(".select-items").classList.add("select-hide");
        }
    }
...
function select_item(select) {
    ...
    if (select_type === 'input') {
        select_type_content = '<div class="select__value _icon-chevron-right"><input autocomplete="off" type="text" name="form[]" value="' + select_selected_option.innerHTML + '" data-error="Ошибка" data-value="' + select_selected_option.innerHTML + '" class="select__input"></div>';
    } else {
        select_type_content = '<div class="select__value _icon-chevron-right"><span>' + select_selected_option.innerHTML + '</span></div>';
    }
